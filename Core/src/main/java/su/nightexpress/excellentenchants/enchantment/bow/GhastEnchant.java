...
public class GhastEnchant extends GameEnchantment implements BowEnchant {
    ...
    public boolean onShoot(@NotNull EntityShootBowEvent event, @NotNull LivingEntity shooter, @NotNull ItemStack bow, int level) {
        ...
        if (EnchantUtils.contains(bow, Enchantment.MULTISHOT)) {
            ...
            this.plugin.runAtEntity(fireball, () -> fireball.setVelocity(projectile.getVelocity().normalize().multiply(0.5f)));
        }
        else {
            fireball = shooter.launchProjectile(Fireball.class);
            this.plugin.runAtEntity(fireball, () -> fireball.setDirection(projectile.getVelocity()));
        }
